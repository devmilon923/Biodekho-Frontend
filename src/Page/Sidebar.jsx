...
const Sidebar = () => {
  ...
  const NavItem = ({ to, icon: Icon, label }) => (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `flex items-center gap-3 py-2.5 px-4 rounded-lg transition duration-200 text-sm ${
          isActive
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed z-40 w-64 h-screen transform top-0 left-0 transition-transform duration-300 ease-in-out overflow-y-auto bg-custom-gradient text-white ${
      ...
      >
        <div className="flex flex-col h-full justify-between px-4 py-6">
          ...
          <div className="mb-6">
            <Link to="/">
              <h1 className="text-xl font-bold p-3 text-center rounded-lg shadow-sm bg-white/10 text-white tracking-wide">
                Biodekho
              </h1>
            </Link>
          </div>
